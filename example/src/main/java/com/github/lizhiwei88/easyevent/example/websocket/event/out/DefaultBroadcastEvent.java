 ...
 * @author lizhiwei
 **/
public class DefaultBroadcastEvent implements OutBoundEvent<WebSocketSession> {

    private String p;

    public DefaultBroadcastEvent(String p) {
        this.p = p;
    }
    ...
    public void execute(WebSocketSession client) {
        try {
            client.sendMessage(new TextMessage("=== you are default. DefaultBroadcast"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
